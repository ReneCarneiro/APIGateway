...
public class ProdutoSvc {
    ...
    private ProdutoSvc ProdutoRepository;
    ...
    // metodo para buscar um produto pela descricao
    public Produto RecuperarProduto1(String codigo) {
        if (!this.dados.containsKey(codigo)) {
            return null;
        }
    ...
    public Produto AtualizarProduto(String codigo, Produto produto) throws CodigoProdutoObrigatorioException {
        if (!this.dados.containsKey(codigo)) {
            return null;
        }
        ...
        if (produto.getCodigo() != null && !produto.getCodigo().isEmpty()) {
            produtoAux.setCodigo(produto.getCodigo());
        }
